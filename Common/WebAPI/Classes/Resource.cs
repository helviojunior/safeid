...
namespace IAM.WebAPI.Classes
{
    ...
    /// </summary>
    internal class Resource : APIBase
    {
        ...
        public override Object Process(SqlConnection sqlConnection, Int64 enterpriseId, String method, String auth, Dictionary<String, Object> parameters)
        {
            ...
            {
                case "new":
                    return newresource(sqlConnection, parameters);
                    break;
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private Dictionary<String, Object> newresource(SqlConnection sqlConnection, Dictionary<String, Object> parameters)
        {
            ...
            par.Add("@resource_name", typeof(String), name.Length).Value = name;
            par.Add("@proxy_id", typeof(Int64)).Value = proxyid;
            par.Add("@context_id", typeof(Int64)).Value = contextid;

            DataTable dtUsers = ExecuteDataTable(sqlConnection, "sp_new_resource", CommandType.StoredProcedure, par, null);
            if (dtUsers == null)
            {
        ...
        private Dictionary<String, Object> get(SqlConnection sqlConnection, Dictionary<String, Object> parameters)
        {
            ...
            DataTable dtResource = ExecuteDataTable(sqlConnection, "select r.*, c.enterprise_id, c.name as context_name, p.name as proxy_name, resource_plugin_qty = (select COUNT(distinct rp.id) from resource_plugin rp with(nolock) where rp.resource_id = r.id) from resource r with(nolock) inner join context c with(nolock) on c.id = r.context_id inner join proxy p on p.id = r.proxy_id where c.enterprise_id = @enterprise_id and r.id = @resource_id", CommandType.Text, par, null);
            if (dtResource == null)
            {
                Error(ErrorType.InternalError, "", "", null);
                return null;
        ...
        private Boolean delete(SqlConnection sqlConnection, Dictionary<String, Object> parameters)
        {
            ...
            ExecuteNonQuery(sqlConnection, "delete from resource where id = @resource_id", CommandType.Text, par);
            using (IAMDatabase db = new IAMDatabase(sqlConnection))
                db.AddUserLog(LogKey.Resource_Deleted, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, 0, 0, "Resource " + dtResource.Rows[0]["name"] + " deleted", "");
        ...
        private Dictionary<String, Object> change(SqlConnection sqlConnection, Dictionary<String, Object> parameters)
        {
                    ...
                    case "contextid":
                        ...
                        if (!String.IsNullOrWhiteSpace(context))
                        {
                            ...
                            if (context != dtResource.Rows[0]["context_id"].ToString())
                            {
                                ...
                                DataTable dtCtx = ExecuteDataTable(sqlConnection, "select * from context where enterprise_id = @enterprise_id and id = " + context, CommandType.Text, par, null);
                                ...
                                DataTable dtIdent = ExecuteDataTable(sqlConnection, "select COUNT(distinct i.id) qty from resource r with(nolock) inner join context c with(nolock) on r.context_id = c.id inner join resource_plugin rp with(nolock) on rp.resource_id = r.id inner join [identity] i with(nolock) on rp.id = i.resource_plugin_id where r.id = " + resourceid, CommandType.Text, par, null);
                    ...
                    case "proxyid":
                        ...
                        if (!String.IsNullOrWhiteSpace(proxy))
                        {
                            ...
                            if (proxy != dtResource.Rows[0]["proxy_id"].ToString())
                            {
                                ...
                                //Verifica se o contexto pertence a mesma empresa
                                DataTable dtProxy = ExecuteDataTable(sqlConnection, "select * from proxy where enterprise_id = @enterprise_id and id = " + proxy, CommandType.Text, par, null);
            ...
            if (update)
            {
                ...
                AddUserLog(sqlConnection, LogKey.Role_Changed, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, 0, 0, "Resource changed", String.Join("\r\n", log));
            }

            //Atualiza a busca com os dados atualizados
            dtResource = ExecuteDataTable(sqlConnection, "select r.*, c.enterprise_id, c.name as context_name, p.name as proxy_name, resource_plugin_qty = (select COUNT(distinct rp.id) from resource_plugin rp with(nolock) where rp.resource_id = r.id) from resource r with(nolock) inner join context c with(nolock) on c.id = r.context_id inner join proxy p on p.id = r.proxy_id where c.enterprise_id = @enterprise_id and r.id = @resource_id", CommandType.Text, par, null);
