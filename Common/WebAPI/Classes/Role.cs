...
namespace IAM.WebAPI.Classes
{
    ...
    internal class Role : APIBase
    {
        ...
        private Dictionary<String, Object> newrole(SqlConnection sqlConnection, Dictionary<String, Object> parameters)
        {
            ...
            par.Add("@role_name", typeof(String), name.Length).Value = name;
            par.Add("@parent_id", typeof(Int64)).Value = parentid;
            par.Add("@context_id", typeof(Int64)).Value = contextid;

            DataTable dtUsers = ExecuteDataTable(sqlConnection, "sp_new_role", CommandType.StoredProcedure, par, null);
            if (dtUsers == null)
            {
        ...
        private Dictionary<String, Object> get(SqlConnection sqlConnection, Dictionary<String, Object> parameters)
        {
            ...
            DataTable dtUsers = ExecuteDataTable(sqlConnection, "select r.*, c.enterprise_id, entity_qty = (select COUNT(distinct i.entity_id) from identity_role ir inner join [identity] i with(nolock) on ir.identity_id = i.id where ir.role_id = r.id) from role r inner join context c with(nolock) on c.id = r.context_id where c.enterprise_id = @enterprise_id and r.id = @role_id order by r.name", CommandType.Text, par, null);
            if (dtUsers == null)
            {
                Error(ErrorType.InternalError, "", "", null);
                return null;
        ...
        private Boolean delete(SqlConnection sqlConnection, Dictionary<String, Object> parameters)
        {
            ...
            DataTable dtUsers = ExecuteDataTable(sqlConnection, "select c.enterprise_id, r.name as role_name, ir.*, i.entity_id from role r inner join context c with(nolock) on c.id = r.context_id left join identity_role ir on r.id = ir.role_id left join [identity] i with(nolock) on ir.identity_id = i.id where c.enterprise_id = @enterprise_id and r.id = @role_id", CommandType.Text, par, null);
            if (dtUsers == null)
            {
                Error(ErrorType.InternalError, "", "", null);
                return false;
            ...
            ExecuteNonQuery(sqlConnection, "delete from role where id = @role_id", CommandType.Text, par);
            AddUserLog(sqlConnection, LogKey.Role_Deleted, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, 0, 0, "Role " + dtUsers.Rows[0]["role_name"] + " deleted", "");


            foreach (DataRow dr in dtUsers.Rows)
                if ((dr["identity_id"] != DBNull.Value) && (dr["entity_id"] != DBNull.Value))
                {
                    AddUserLog(sqlConnection, LogKey.User_IdentityRoleUnbind, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, (Int64)dr["entity_id"], (Int64)dr["identity_id"], "Identity unbind to role " + dr["role_name"], "");
        ...
        private Boolean deleteallusers(SqlConnection sqlConnection, Dictionary<String, Object> parameters)
        {
            ...
            ExecuteNonQuery(sqlConnection, "delete from identity_role where role_id = @role_id", CommandType.Text, par);
        ...
        private Boolean deleteuser(SqlConnection sqlConnection, Dictionary<String, Object> parameters)
        {
            ...
            DataTable dtUsers = ExecuteDataTable(sqlConnection, "select c.enterprise_id, r.name as role_name, ir.*, i.entity_id from role r inner join context c with(nolock) on c.id = r.context_id left join identity_role ir on r.id = ir.role_id left join [identity] i with(nolock) on ir.identity_id = i.id and i.entity_id = @entity_id where c.enterprise_id = @enterprise_id and r.id = @role_id", CommandType.Text, par, null);
            if (dtUsers == null)
            {
                Error(ErrorType.InternalError, "", "", null);
                return false;
            ...
            foreach (DataRow dr in dtUsers.Rows)
                ...
                {
                    AddUserLog(sqlConnection, LogKey.User_IdentityRoleUnbind, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, (Int64)dr["entity_id"], (Int64)dr["identity_id"], "Identity unbind to role " + dr["role_name"], "");
                    ExecuteNonQuery(sqlConnection, "delete from identity_role where role_id = @role_id and identity_id = " + dr["identity_id"], CommandType.Text, par);
                    ExecuteNonQuery(sqlConnection, "insert into deploy_now (entity_id) values(" + dr["entity_id"] + ")", CommandType.Text,null,  null);
                }
        ...
        private Boolean adduser(SqlConnection sqlConnection, Dictionary<String, Object> parameters)
        {
            ...
            foreach (Int64 u in users)
            {
                ...
                DataTable dtRet = ExecuteDataTable(sqlConnection, "sp_insert_entity_to_role", CommandType.StoredProcedure, par2);

                if ((dtRet != null) && (dtRet.Rows.Count > 0))
                {
                    AddUserLog(sqlConnection, LogKey.User_IdentityRoleBind, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, u, (Int64)dtRet.Rows[0]["identity_id"], "Identity bind to role " + dtRet.Rows[0]["role_name"].ToString(), "");
                }
            }
        ...
        private Dictionary<String, Object> change(SqlConnection sqlConnection, Dictionary<String, Object> parameters)
        {
            ...
            if (update)
            {
                ...
                AddUserLog(sqlConnection, LogKey.Role_Changed, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, 0, 0, "Role changed", String.Join("\r\n", log));
            }

            //Atualiza a busca com os dados atualizados
            dtUsers = ExecuteDataTable(sqlConnection, "select r.*, c.enterprise_id, entity_qty = (select COUNT(distinct i.entity_id) from identity_role ir inner join [identity] i with(nolock) on ir.identity_id = i.id where ir.role_id = r.id) from role r inner join context c with(nolock) on c.id = r.context_id where c.enterprise_id = @enterprise_id and r.id = @role_id order by r.name", CommandType.Text, par, null);
