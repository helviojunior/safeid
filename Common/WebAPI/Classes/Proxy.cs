...
namespace IAM.WebAPI.Classes
{
    ...
    internal class Proxy : APIBase
    {
        ...
        public override Object Process(SqlConnection sqlConnection, Int64 enterpriseId, String method, String auth, Dictionary<String, Object> parameters)
        {
            ...
            {
                case "new":
                    return newproxy(sqlConnection, parameters);
                    break;
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private Dictionary<String, Object> newproxy(SqlConnection sqlConnection, Dictionary<String, Object> parameters)
        {
            ...
            par.Add("@proxy_name", typeof(String), name.Length).Value = name;

            DataTable dtProxy = ExecuteDataTable(sqlConnection, "sp_new_proxy", CommandType.StoredProcedure, par, null);
            if (dtProxy == null)
            {
        ...
        private Dictionary<String, Object> get(SqlConnection sqlConnection, Dictionary<String, Object> parameters)
        {
            ...
            DataTable dtPlugin = ExecuteDataTable(sqlConnection, "select p.*, resource_qty = (select COUNT(distinct r1.proxy_id) from resource r1 with(nolock) where r1.proxy_id = p.id) from proxy p with(nolock) where p.enterprise_id = @enterprise_id and p.id = @proxy_id", CommandType.Text, par, null);
            if (dtPlugin == null)
            {
                Error(ErrorType.InternalError, "", "", null);
                return null;
        ...
        private Boolean delete(SqlConnection sqlConnection, Dictionary<String, Object> parameters)
        {
            ...
            DataTable dtProxy = ExecuteDataTable(sqlConnection, "select p.*, resource_qty = (select COUNT(distinct r1.proxy_id) from resource r1 with(nolock) where r1.proxy_id = p.id) from proxy p where (p.enterprise_id = @enterprise_id or p.enterprise_id = 0) and p.id = @proxy_id", CommandType.Text, par, null);
            if (dtProxy == null)
            {
                Error(ErrorType.InternalError, "", "", null);
                return false;
            ...
            ExecuteNonQuery(sqlConnection, "delete from proxy where id = @proxy_id", CommandType.Text, par);
            using (IAMDatabase db = new IAMDatabase(sqlConnection))
                db.AddUserLog(LogKey.Proxy_Deleted, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, 0, 0, "Proxy " + dtProxy.Rows[0]["name"] + " deleted", "");
        ...
        private Dictionary<String, Object> change(SqlConnection sqlConnection, Dictionary<String, Object> parameters)
        {
            ...
            if (update)
            {
                ...
                AddUserLog(sqlConnection, LogKey.Proxy_Changed, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, 0, 0, "Proxy changed", String.Join("\r\n", log));
            }

            //Atualiza a busca com os dados atualizados
            dtProxy = ExecuteDataTable(sqlConnection, "select p.*, resource_qty = (select COUNT(distinct r1.proxy_id) from resource r1 with(nolock) where r1.proxy_id = p.id) from proxy p where (p.enterprise_id = @enterprise_id or p.enterprise_id = 0) and p.id = @proxy_id", CommandType.Text, par, null);
