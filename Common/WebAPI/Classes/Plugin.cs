...
namespace IAM.WebAPI.Classes
{
    ...
    internal class Plugin : APIBase
    {
        ...
        public override Object Process(SqlConnection sqlConnection, Int64 enterpriseId, String method, String auth, Dictionary<String, Object> parameters)
        {
            ...
            {
                case "new":
                    return newplugin(sqlConnection, parameters);
                    break;
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private Dictionary<String, Object> newplugin(SqlConnection sqlConnection, Dictionary<String, Object> parameters)
        {
        ...
        private Dictionary<String, Object> get(SqlConnection sqlConnection, Dictionary<String, Object> parameters)
        {
            ...
            DataTable dtPlugin = ExecuteDataTable(sqlConnection, "select p.*, resource_plugin_qty = (select COUNT(distinct rp1.plugin_id) from resource_plugin rp1 where rp1.plugin_id = p.id) from plugin p with(nolock) where (p.enterprise_id = @enterprise_id or p.enterprise_id = 0) and p.id = @plugin_id", CommandType.Text, par, null);
            if (dtPlugin == null)
            {
                Error(ErrorType.InternalError, "", "", null);
                return null;
        ...
        private Boolean delete(SqlConnection sqlConnection, Dictionary<String, Object> parameters)
        {
            ...
            DataTable dtPlugin = ExecuteDataTable(sqlConnection, "select p.*, resource_plugin_qty = (select COUNT(distinct rp1.plugin_id) from resource_plugin rp1 where rp1.plugin_id = p.id) from plugin p with(nolock) where p.enterprise_id = @enterprise_id and p.id = @plugin_id", CommandType.Text, par, null);
            if (dtPlugin == null)
            {
                Error(ErrorType.InternalError, "", "", null);
                return false;
            ...
            try
            {
                ...
                pluginsDir = new DirectoryInfo(GetDBConfig(sqlConnection, "pluginFolder"));
            ...
            try
            {
                ExecuteNonQuery(sqlConnection, "delete from plugin where id = @plugin_id", CommandType.Text,par,  trans);
                AddUserLog(sqlConnection, LogKey.Plugin_Deleted, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, 0, 0, "Plugin " + dtPlugin.Rows[0]["name"] + " deleted", "", trans);
