...
using System.Web;
using System.Web.UI;
//using SafeTrend.Audit.GlobalDefs;
...
namespace SafeTrend.WebAPI
{
    ...
    public static class WebPageAPI
    {
        ...
        private static ExternalAccessControl GetDelegateInstance(SqlConnection conn, Page page, Int64 enterpriseId)
        {
            ExternalAccessControl eAuth = new ExternalAccessControl(delegate(String method, String auth, AccessControl preCtrl, Dictionary<String, Object> parameters)
            {
                ...
                {
                    return new AccessControl(login.Id, RBAC.UserCan(conn, login.Id, enterpriseId, "admin", method));
                }
                else
                {
                    return preCtrl;
        ...
        public static String ExecuteLocal(SqlConnection conn, Page page, String jRequest)
        {
            return ExecuteLocal(conn, page, jRequest, null);
        }
        ...
        /// <param name="page">Página na qual a requisição foi iniciada</param>
        /// <param name="jRequest">Texto no formato JSON da requisição</param>
        public static String ExecuteLocal(SqlConnection conn,  Page page, String jRequest, ExecutionLog logDelegate)
        {
            try
            {
                ...
                ExecutionLog eLogs = new ExecutionLog(delegate(Boolean success, Int64 enterpriseIdLog, String method, AccessControl acl, String jRequestLog, String jResponseLog)
                {
                    ...
                    if (!success)
                    {
                        using (IAMDatabase db = new IAMDatabase(conn))
                            db.AddUserLog(LogKey.Debug, null, "API", UserLogLevel.Debug, 0, enterpriseIdLog, 0, 0, 0, 0, 0, "API Call (" + method + "). Result success? " + success, "{\"Request\":" + jRequestLog + ", \"Response\":" + jResponseLog + "}", acl.EntityId, null);
                    }
                ...
                CJSONrpc jsonRpc = new CJSONrpc(conn, jRequest, enterpriseId);

                ExternalAccessControl eAuth = GetDelegateInstance(conn, page, enterpriseId);
        ...
        /// <param name="conn">Conexão com banco de dados MS-SQL</param>
        /// <param name="page">Página na qual a requisição foi iniciada</param>
        public static void Execute(SqlConnection conn, Page page)
        {
            Execute(conn, page, null);
        }
        ...
        /// <param name="conn">Conexão com banco de dados MS-SQL</param>
        /// <param name="page">Página na qual a requisição foi iniciada</param>
        public static void Execute(SqlConnection conn, Page page, ExecutionLog logDelegate)
        {
            ...
            if (allowed_content[contentType] == "json-rpc")
            {
                ...
                try
                {
                    ...
                    using (StreamReader reader = new StreamReader(stm, Encoding.UTF8))
                    {
                        ...
                        CJSONrpc jsonRpc = new CJSONrpc(conn, rData, enterpriseId);
