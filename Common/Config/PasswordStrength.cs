...
{
    public class UserPasswordStrengthResult
    {
        public Boolean LengthError { get; set; }
        public Boolean UpperCaseError { get; set; }
        public Boolean LowerCaseError { get; set; }
        public Boolean DigitError { get; set; }
        public Boolean SymbolError { get; set; }
        ...
        public Boolean HasLength { get { return hasLength; } internal set { hasLength = value; } }
        public Boolean HasUpperCase { get { return hasUpperCase; } internal set { hasUpperCase = value; } }
        public Boolean HasLowerCase { get { return hasLowerCase; } internal set { hasLowerCase = value; } }
        public Boolean HasDigit { get { return hasDigit; } internal set { hasDigit = value; } }
        public Boolean HasSymbol { get { return hasSymbol; } internal set { hasSymbol = value; } }
        public Boolean HasNamePart { get { return hasNamePart; } internal set { hasNamePart = value; } }


        public Boolean HasError { get { return LengthError || UpperCaseError || LowerCaseError || DigitError || SymbolError || NameError; } }

        public UserPasswordStrengthResult()
        {
            ...
            this.hasNamePart = false;
        }
    }

    public class UserPasswordStrength : IAMDatabase
    {
        ...
        private Boolean requiresNoName;
        private String fullName;

        public UserPasswordStrength(SqlConnection conn, Int64 contextId, String fullName)
        {
            //Carrega a configuração do contexto
            this.Connection = conn;

            DataTable dtConfig = ExecuteDataTable("select c.pwd_length, c.pwd_upper_case, c.pwd_lower_case, c.pwd_digit, c.pwd_symbol, c.pwd_no_name from context c with(nolock) where c.id = " + contextId);
            if ((dtConfig == null) || (dtConfig.Rows.Count == 0))
                throw new Exception("Entity/context not found");

            this.fullName = fullName;
            this.minLength = (Int32)dtConfig.Rows[0]["pwd_length"];
            this.requiresUpperCase = (Boolean)dtConfig.Rows[0]["pwd_upper_case"];
            this.requiresLowerCase = (Boolean)dtConfig.Rows[0]["pwd_lower_case"];
            this.requiresDigit = (Boolean)dtConfig.Rows[0]["pwd_digit"];
            this.requiresSymbol = (Boolean)dtConfig.Rows[0]["pwd_symbol"];
            this.requiresNoName = (Boolean)dtConfig.Rows[0]["pwd_no_name"];

        }

        public UserPasswordStrength(SqlConnection conn, Int64 entityId)
        {
            //Carrega a configuração do contexto do usuário
            this.Connection = conn;

            DataTable dtConfig = ExecuteDataTable("select e.id, e.full_name, c.pwd_length, c.pwd_upper_case, c.pwd_lower_case, c.pwd_digit, c.pwd_symbol, c.pwd_no_name from context c with(nolock) inner join entity e with(nolock) on c.id = e.context_id where e.id = " + entityId);
            if ((dtConfig == null) || (dtConfig.Rows.Count == 0))
                throw new Exception("Entity/context not found");

            this.fullName = (String)dtConfig.Rows[0]["full_name"];
            this.minLength = (Int32)dtConfig.Rows[0]["pwd_length"];
            this.requiresUpperCase = (Boolean)dtConfig.Rows[0]["pwd_upper_case"];
            this.requiresLowerCase = (Boolean)dtConfig.Rows[0]["pwd_lower_case"];
            this.requiresDigit = (Boolean)dtConfig.Rows[0]["pwd_digit"];
        ...
        public UserPasswordStrengthResult CheckPassword(String password)
        {
            UserPasswordStrengthResult ret = new UserPasswordStrengthResult();

            PasswordStrength pwdS = new PasswordStrength();
            pwdS.SetPassword(password);

            if (password.Length < this.minLength)
                ret.LengthError = true;


            ret.HasUpperCase = pwdS.HasUpperCase;
            ret.HasLowerCase = pwdS.HasLowerCase;
            ret.HasDigit = pwdS.HasDigit;
            ret.HasSymbol = pwdS.HasSymbol;
            if (!String.IsNullOrWhiteSpace(fullName))
                ret.HasNamePart = PasswordStrength2.Check(fullName, password);

            ret.UpperCaseError = (this.requiresUpperCase && !ret.HasUpperCase);
            ret.LowerCaseError = (this.requiresLowerCase && !ret.HasLowerCase);
            ret.DigitError = (this.requiresDigit && !ret.HasDigit);
            ret.SymbolError = (this.requiresSymbol && !ret.HasSymbol);

            ret.NameError = (this.requiresNoName && ret.HasNamePart);
    ...
    public static class PasswordStrength2
    {
