...
namespace IAM.Config
{
    ...
    public class IgnoreRules : IAMDatabase, IDisposable
    {
        ...
        public void GetDBConfig(SqlConnection conn)
        {
            ...
            DataTable dt = ExecuteDataTable("select rp.resource_id, p.uri, f.id filter_id, f.name from resource_plugin_ignore_filter rpf with(nolock) inner join filters f with(nolock) on f.id = rpf.filter_id inner join resource_plugin rp with(nolock) on rpf.resource_plugin_id = rp.id inner join plugin p with(nolock) on rp.plugin_id = p.id");
