...
namespace CAS.Web
{
    ...
    public class CASUtils
    {
        ...
        public static void ClearCookie(Page page)
        {
        ...
        public static EnterpriseData EnterpriseByService(Page page, String service)
        {
            ...
            try
            {
                par = DB.GetSqlParameterObject();
                par.Add("@svc", SqlDbType.VarChar, service.Length).Value = service.TrimEnd("/".ToCharArray()).Replace("https://", "//").Replace("http://", "//").Trim();

                DataTable dt = DB.Select("select * from [cas_service] s inner join enterprise e on s.enterprise_id = e.id where s.service_uri = @svc", par);
        ...
        public static Boolean ServiceExists(String service)
        {
            ...
            try
            {
                ...
                DataTable dt = DB.Select("select * from [cas_service] where service_uri = @svc", par);
        ...
        public static LoginResult Grant(String service, String ticket, Boolean renew)
        {
            ...
            {
                par = DB.GetSqlParameterObject();
                par.Add("@tgc", SqlDbType.VarChar, ticket.Length).Value = ticket.Trim();
                par.Add("@svc", SqlDbType.VarChar, service.Length).Value = service.TrimEnd("/".ToCharArray()).Replace("https://", "//").Replace("http://", "//").Trim();

                Int64 userId = 0;

                //Realiza a busca pelo ticket no mesmo serviço
                DataTable dt = DB.Select("select * from [cas_entity_ticket] t inner join [cas_service] s on t.service_id = s.id where t.expire_date > getdate() " + (renew ? " and create_by_credentials = 1 " : "") + " and s.service_uri = @svc and t.grant_ticket = @tgc", par);
                if ((dt != null) && (dt.Rows.Count > 0))
                {
                ...
                else
                {
                    ...
                    dt = DB.Select("select * from [cas_entity_ticket] t inner join [cas_service] s on t.service_id = s.id where t.expire_date > getdate() " + (renew ? " and create_by_credentials = 1 " : "") + " and t.grant_ticket = @tgc", par);
                    if ((dt != null) && (dt.Rows.Count > 0))
                    {
                        par.Add("@entity_id", SqlDbType.BigInt).Value = (Int64)dt.Rows[0]["entity_id"];
                        par.Add("@grant_ticket", SqlDbType.VarChar, dt.Rows[0]["grant_ticket"].ToString().Length).Value = dt.Rows[0]["grant_ticket"].ToString().Trim();
                        par.Add("@long_ticket", SqlDbType.VarChar, dt.Rows[0]["long_ticket"].ToString().Length).Value = dt.Rows[0]["long_ticket"].ToString().Trim();

                        //Cria o ticket
                        DB.ExecuteSQL("insert into cas_entity_ticket ([entity_id],[service_id],[grant_ticket],[long_ticket],[create_by_credentials]) select @entity_id, s.id, @grant_ticket, @long_ticket, 0 from cas_service s where s.service_uri = @svc", par, CommandType.Text);
                        userId = (Int64)dt.Rows[0]["entity_id"];
                    }
                    else
                    {
                ...
                if (userId > 0)
                {
                    ...
                    DataTable dtEntity = DB.Select("select distinct l.id, l.alias, l.full_name, l.login, l.enterprise_id, l.password, l.must_change_password, s.id as service_id, s.service_uri, et.grant_ticket, et.long_ticket from vw_entity_logins l  inner join dbo.cas_entity_ticket et on et.entity_id = l.id inner join cas_service s on l.enterprise_id = s.enterprise_id and et.service_id = s.id where et.grant_ticket = @tgc and s.service_uri = @svc", par);
        ...
        static public LoginResult Grant(Page page, String username, String password)
        {
            ...
            try
            {
                ...
                SqlParameterCollection par = DB.GetSqlParameterObject();
                par.Add("@login", SqlDbType.VarChar, username.Length).Value = username;
                par.Add("@svc", SqlDbType.VarChar, svc.Length).Value = svc;

                DataTable tmp = DB.Select("select distinct l.id, l.alias, l.full_name, l.login, l.enterprise_id, l.password, l.must_change_password, s.id as service_id, c.service_uri, c.grant_ticket, c.long_ticket from vw_entity_logins l inner join cas_service s on l.enterprise_id = s.enterprise_id left join (select * from cas_entity_ticket c1 inner join cas_service s on s.id = c1.service_id) c on l.id = c.entity_id and c.service_uri = @svc where l.deleted = 0 and l.locked = 0 and (l.login = @login or l.value = @login) and s.service_uri = @svc", par);
                ...
                if ((tmp != null) && (tmp.Rows.Count > 0))
                {
                    foreach (DataRow dr in tmp.Rows)
                    {
                        ...
                        using (ServerKey sk = new ServerKey(conn))
                        using (CryptApi cApi = CryptApi.ParsePackage(sk.ServerPKCS12Cert, Convert.FromBase64String(dr["password"].ToString())))
                            if (Encoding.UTF8.GetString(cApi.clearData) == password)
                            {
                                ...
                                DB.ExecuteSQL("update entity set last_login = getdate() where id = " + l.Id, null, CommandType.Text);

                                if (tmp.Rows[0]["service_uri"] == DBNull.Value)
                                    DB.ExecuteSQL("insert into cas_entity_ticket ([entity_id],[service_id],[grant_ticket],[long_ticket],[create_by_credentials]) VALUES (" + l.Id + ", " + tmp.Rows[0]["service_id"].ToString() + ", '" + l.CASGrantTicket + "', '" + l.CASLongTicket + "',1)", null, CommandType.Text);
                                else
                                    DB.ExecuteSQL("update cas_entity_ticket set grant_ticket = '" + l.CASGrantTicket + "', long_ticket = '" + l.CASLongTicket + "', expire_date = dateadd(day,1,getdate()), create_by_credentials = 1 where entity_id = " + l.Id + " and service_id = " + tmp.Rows[0]["service_id"].ToString(), null, CommandType.Text);
    ...
    {
        public Boolean Status { get; internal set; }
        public String Text { get; internal set; }
        public Boolean ChangePassword { get; set; }
        public LoginData LoginData { get; set; }

        public LoginResult(Boolean Status, String Text) :
            this(Status, Text, false, null) { }

        public LoginResult(Boolean Status, String Text, Boolean ChangePassword) :
            this(Status, Text, ChangePassword, null) { }


        public LoginResult(Boolean Status, String Text, Boolean ChangePassword, LoginData LoginData)
        {
            this.Status = Status;
            this.Text = Text;
