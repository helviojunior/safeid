...
{
    internal class Logs : IAMAPIBase
    {
        ...
        private List<Object> list(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            sql += "  SELECT";
            sql += "    ROW_NUMBER() OVER (ORDER BY l.date desc) AS [row_number], l.*, res.name resource_name, p.name plugin_name";
            sql += "    from logs l with(nolock) left join [identity] i with(nolock) on i.id = l.identity_id left join resource res with(nolock) on res.id = l.resource_id left join plugin p with(nolock) on p.id = l.plugin_id";
            sql += "  WHERE";
