...
namespace IAM.WebAPI.Classes
{
    ...
    /// </summary>
    internal class ResourcePlugin : IAMAPIBase
    {
        ...
        public override Object iProcess(IAMDatabase database, Int64 enterpriseId, String method, String auth, Dictionary<String, Object> parameters)
        {
            ...
            {
                case "new":
                    return newresourceplugin(database, parameters);
                    break;

                case "get":
                    return get(database, parameters, false);
                    break;

                case "clone":
                    return clone(database, parameters, false);
                    break;

                case "parameters":
                    return getparameters(database, parameters);
                    break;

                case "mapping":
                    return getmapping(database, parameters);
                    break;

                case "lockrules":
                    return getlockrules(database, parameters);
                    break;

                case "ignore":
                    return getignore(database, parameters);
                    break;

                case "roles":
                    return getroles(database, parameters);
                    break;

                case "schedules":
                    return getschedules(database, parameters);
                    break;

                case "fieldsfetch":
                    return fieldsfetch(database, parameters);
                    break;
                ...
                case "changeparameters":
                    return changeparameters(database, parameters);
                    break;

                case "changemapping":
                    return changemapping(database, parameters);
                    break;

                case "changerole":
                    return changerole(database, parameters);
                    break;

                case "changelockrules":
                    return changelockrules(database, parameters);
                    break;

                case "changeignore":
                    return changeignore(database, parameters);
                    break;

                case "changeschedules":
                    return changeschedules(database, parameters);
                    break;
                ...
                case "enable":
                    return enable(database, parameters);
                    break;

                case "deploy":
                    return deploy(database, parameters);
                    break;

                case "disable":
                    return disable(database, parameters);
                    break;

                case "newfetch":
                    return newfetch(database, parameters);
                    break;

                case "deletefetch":
                    return deletefetch(database, parameters);
                    break;

                case "identity":
                    return identity(database, parameters);
                    break;

                case "addidentity":
                    return addidentity(database, parameters);
                    break;
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private Dictionary<String, Object> newresourceplugin(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            par.Add("@mail_domain", typeof(String)).Value = mail_domain;

            DataTable dtUsers = database.ExecuteDataTable( "sp_new_resource_plugin", CommandType.StoredProcedure, par, null);
            if (dtUsers == null)
            {
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private List<Dictionary<String, Object>> getroles(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            DataTable dtResourcePlugin = database.ExecuteDataTable( "SELECT rp.id FROM resource_plugin rp with(nolock) INNER JOIN resource r with(nolock) ON r.id = rp.resource_id INNER JOIN plugin p with(nolock) ON p.id = rp.plugin_id INNER JOIN context c with(nolock) ON r.context_id = c.id  WHERE rp.id = @resource_plugin_id AND c.enterprise_id = @enterprise_id", CommandType.Text, par, null);
            ...
            DataTable dtResourcePluginRole = database.ExecuteDataTable( "select r.* from resource_plugin_role rpr with(nolock) inner join role r with(nolock) on rpr.role_id = r.id WHERE rpr.resource_plugin_id = @resource_plugin_id order by r.name", CommandType.Text, par, null);
            ...
            foreach (DataRow dr1 in dtResourcePluginRole.Rows)
            {
                ...
                DataTable dtResourcePluginRoleAction = database.ExecuteDataTable( "select * from resource_plugin_role_action rpa with(nolock) WHERE rpa.resource_plugin_id = @resource_plugin_id and rpa.role_id = " + dr1["id"], CommandType.Text, par, null);
                ...
                DataTable dtResourcePluginRoleExpression = database.ExecuteDataTable( "select rpf.*, f.name filter_name from resource_plugin_role_filter rpf with(nolock) inner join filters f with(nolock) on rpf.filter_id = f.id WHERE rpf.resource_plugin_id = @resource_plugin_id and rpf.role_id = " + dr1["id"], CommandType.Text, par, null);
                if (dtResourcePluginRoleExpression != null)
                {
                    ...
                    foreach (DataRow dr2 in dtResourcePluginRoleExpression.Rows)
                    {
                        ...
                        DataTable dt3 = database.ExecuteDataTable( "select f.*, f1.name field_name, f1.data_type from filters_conditions f with(nolock) inner join field f1 with(nolock) on f1.id = f.field_id where f.filter_id = " + dr2["filter_id"]);
                ...
                DataTable dtResourcePluginRoleAcl = database.ExecuteDataTable( "select * from resource_plugin_role_time_acl rpacl with(nolock) WHERE rpacl.resource_plugin_id = @resource_plugin_id and rpacl.role_id = " + dr1["id"], CommandType.Text, par, null);
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private List<Dictionary<String, Object>> getmapping(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            DataTable dtResourcePluginMapping = database.ExecuteDataTable( "select rpm.*, f.name field_name, f.data_type field_data_type from resource_plugin_mapping rpm with(nolock) inner join field f on rpm.field_id = f.id WHERE rpm.resource_plugin_id = @resource_plugin_id order by f.name", CommandType.Text, par, null);
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private List<Dictionary<String, Object>> getlockrules(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            DataTable dtResourcePluginLock = database.ExecuteDataTable( "select rplf.*, f.name filter_name from resource_plugin_lock_filter rplf with(nolock) inner join filters f with(nolock) on rplf.filter_id = f.id WHERE rplf.resource_plugin_id = @resource_plugin_id order by f.name", CommandType.Text, par, null);
            ...
            foreach (DataRow dr1 in dtResourcePluginLock.Rows)
            {
                ...
                DataTable dt2 = database.ExecuteDataTable( "select f.*, f1.name field_name, f1.data_type from filters_conditions f with(nolock) inner join field f1 with(nolock) on f1.id = f.field_id where f.filter_id = " + dr1["filter_id"]);
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private List<Dictionary<String, Object>> getignore(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            DataTable dtResourcePluginIgnore = database.ExecuteDataTable( "select rpif.*, f.name filter_name from resource_plugin_ignore_filter rpif with(nolock) inner join filters f with(nolock) on rpif.filter_id = f.id WHERE rpif.resource_plugin_id = @resource_plugin_id order by f.name", CommandType.Text, par, null);
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private List<Dictionary<String, Object>> getschedules(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            DataTable dtResourcePluginSchedule = database.ExecuteDataTable( "select * from resource_plugin_schedule rple WHERE rple.resource_plugin_id = @resource_plugin_id order by rple.id", CommandType.Text, par, null);
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private List<Dictionary<String, Object>> fieldsfetch(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            DataTable dtResourcePluginFetch = database.ExecuteDataTable( "select * from resource_plugin_fetch rpff WHERE rpff.resource_plugin_id = @resource_plugin_id order by rpff.request_date desc", CommandType.Text, par, null);
                ...
                try
                {
                    if (dr1["response_date"] != DBNull.Value)
                    {
                        ...
                        if (tmp.ContainsKey("result_data"))
                        {
                            ...
                            PluginConnectorBaseFetchResult r = SafeTrend.Json.JSON.Deserialize<PluginConnectorBaseFetchResult>(rDataJson);
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private List<Dictionary<String, Object>> getparameters(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            DataTable dtResourcePluginPar = database.ExecuteDataTable( "select * from resource_plugin_par rpp WHERE rpp.resource_plugin_id = @resource_plugin_id", CommandType.Text, par, null);
            if (dtResourcePlugin == null)
            {
                Error(ErrorType.InternalError, "", "", null);
                return null;
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private Dictionary<String, Object> get(IAMDatabase database, Dictionary<String, Object> parameters, Boolean ignoreCheckConfigErrors)
        {
            ...
            newItem.Add("build_login", dr1["build_login"]);
            newItem.Add("build_mail", dr1["build_mail"]);
            newItem.Add("import_groups", dr1["import_groups"]);
            newItem.Add("import_containers", dr1["import_containers"]);
            newItem.Add("enable_import", dr1["enable_import"]);
            newItem.Add("enable_deploy", dr1["enable_deploy"]);
            newItem.Add("order", dr1["order"]);
            newItem.Add("name_field_id", dr1["name_field_id"]);
            ...
            {
                Dictionary<string, object> cc = CheckConfig(database, resourcepluginid, ignoreCheckConfigErrors);
                if (cc == null)
                {
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private Dictionary<String, Object> clone(IAMDatabase database, Dictionary<String, Object> parameters, Boolean ignoreCheckConfigErrors)
        {
            ...
            try
            {
                ...
                DataTable dtNew = database.ExecuteDataTable( "sp_clone_resourceplugin", CommandType.StoredProcedure, par, trans);
                ...
                return get(database, parameters2, true);
        ...
        private Dictionary<string, object> CheckConfig(IAMDatabase database, Int64 resourcePluginId)
        {
            return CheckConfig(database, resourcePluginId, false);
        }

        private Dictionary<string, object> CheckConfig(IAMDatabase database, Int64 resourcePluginId, Boolean ignoreCheckConfigErrors)
        {
            Dictionary<string, object> result = new Dictionary<string, object>();
            ...
            DataTable dtResourcePlugin = database.ExecuteDataTable( "select p.*, rp.mail_domain, rp.name_field_id, rp.mail_field_id, rp.login_field_id from resource_plugin rp with(nolock) inner join plugin p with(nolock) on p.id = rp.plugin_id WHERE rp.id = @resource_plugin_id", CommandType.Text, par, null);
            if (dtResourcePlugin == null)
            {
            ...
            DataTable dtResourcePluginMapping = database.ExecuteDataTable( "select rpm.*, f.name field_name, f.data_type field_data_type from resource_plugin_mapping rpm with(nolock) inner join field f on rpm.field_id = f.id WHERE (rpm.is_id = 1 or rpm.is_unique_property = 1) and rpm.resource_plugin_id = @resource_plugin_id order by f.name", CommandType.Text, par, null);
            if (dtResourcePluginMapping == null)
            {
                if (!ignoreCheckConfigErrors) Error(ErrorType.InternalError, "", "", null);
                return null;
        ...
        private Boolean delete(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            DataTable dtResource = database.ExecuteDataTable( "select *, qty = (select COUNT(distinct i.id) from [identity] i with(nolock) where i.resource_plugin_id = rp.id) from resource_plugin rp with(nolock) inner join resource r with(nolock) on r.id = rp.resource_id inner join context c with(nolock) on c.id = r.context_id WHERE rp.id = @resource_plugin_id AND c.enterprise_id = @enterprise_id", CommandType.Text, par, null);
            if (dtResource == null)
            {
                Error(ErrorType.InternalError, "", "", null);
                return false;
            ...
            database.ExecuteNonQuery( "delete from resource_plugin where id = @resource_plugin_id", CommandType.Text, par);
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private Boolean enable(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            if (!parameters.ContainsKey("resourcepluginid"))
            {
                Error(ErrorType.InvalidRequest, "Parameter resourcepluginid is not defined.", "", null);
            ...
            DataTable dtResource = database.ExecuteDataTable("SELECT rp.*, (r.name + ' x ' + p.name) as name, r.name resource_name, r.enabled resource_enabled, p.name plugin_name, p1.id proxy_id, p1.last_sync proxy_last_sync, f1.name name_field_name, f2.name mail_field_name, f3.name login_field_name FROM resource_plugin rp with(nolock) INNER JOIN resource r with(nolock) ON r.id = rp.resource_id INNER JOIN plugin p with(nolock) ON p.id = rp.plugin_id INNER JOIN context c with(nolock) ON r.context_id = c.id INNER JOIN proxy p1 on r.proxy_id = p1.id left join field f1 on f1.id = rp.name_field_id left join field f2 on f2.id = rp.mail_field_id left join field f3 on f3.id = rp.login_field_id  WHERE rp.id = @resource_plugin_id AND c.enterprise_id = @enterprise_id", CommandType.Text, par, null);
            if (dtResource == null)
            {
                Error(ErrorType.InternalError, "", "", null);
                return false;
            ...
            Dictionary<String, Object> cc = CheckConfig(database, resourcepluginid);
            ...
            database.ExecuteNonQuery("update proxy set config = 1 where id = " + dtResource.Rows[0]["proxy_id"], CommandType.Text, par, null);


            database.ExecuteNonQuery( "update resource_plugin set enabled = 1 where id = @resource_plugin_id", CommandType.Text, par);
            database.AddUserLog( LogKey.ResourcePlugin_Changed, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, 0, 0, "Resource " + dtResource.Rows[0]["name"] + " enabled", "");
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private Boolean deploy(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            if (!parameters.ContainsKey("resourcepluginid"))
            {
                Error(ErrorType.InvalidRequest, "Parameter resourcepluginid is not defined.", "", null);
            ...
            DataTable dtResourceSc = database.ExecuteDataTable( "select * from resource_plugin_schedule rps WHERE rps.resource_plugin_id = @resource_plugin_id", CommandType.Text, par, null);
            ...
            database.ExecuteNonQuery( "update resource_plugin_schedule set [next] = '1970-01-01 00:00:00' where resource_plugin_id = @resource_plugin_id", CommandType.Text, par);
            database.ExecuteNonQuery( "update proxy set config = 1 where id = " + dtResource.Rows[0]["proxy_id"], CommandType.Text, par);
            database.AddUserLog( LogKey.ResourcePluginDeploy, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, 0, 0, "Resource " + dtResource.Rows[0]["name"] + " marked for deploy now", "");
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private Boolean disable(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            if (!parameters.ContainsKey("resourcepluginid"))
            {
                Error(ErrorType.InvalidRequest, "Parameter resourcepluginid is not defined.", "", null);
            ...
            database.ExecuteNonQuery( "update resource_plugin set enabled = 0 where id = @resource_plugin_id", CommandType.Text, par);
            database.AddUserLog( LogKey.ResourcePlugin_Changed, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, 0, 0, "Resource " + dtResource.Rows[0]["name"] + " disabled", "");
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private Boolean newfetch(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            if (!parameters.ContainsKey("resourcepluginid"))
            {
                Error(ErrorType.InvalidRequest, "Parameter resourcepluginid is not defined.", "", null);
            ...
            database.ExecuteNonQuery( "insert into resource_plugin_fetch (resource_plugin_id) values(@resource_plugin_id)", CommandType.Text, par);
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private Boolean deletefetch(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            if (!parameters.ContainsKey("resourcepluginid"))
            {
                Error(ErrorType.InvalidRequest, "Parameter resourcepluginid is not defined.", "", null);
            ...
            DataTable dtResource = database.ExecuteDataTable( "SELECT rp.id, openfetch = (select COUNT(distinct f.id) from resource_plugin_fetch f where f.resource_plugin_id = rp.id) FROM resource_plugin rp with(nolock) INNER JOIN resource r with(nolock) ON r.id = rp.resource_id INNER JOIN plugin p with(nolock) ON p.id = rp.plugin_id INNER JOIN context c with(nolock) ON r.context_id = c.id INNER JOIN proxy p1 on r.proxy_id = p1.id WHERE rp.id = @resource_plugin_id AND c.enterprise_id = @enterprise_id", CommandType.Text, par, null);
            if (dtResource == null)
            {
                Error(ErrorType.InternalError, "", "", null);
                return false;
            ...
            database.ExecuteNonQuery( "delete from resource_plugin_fetch where resource_plugin_id = @resource_plugin_id and id = " + fetchid, CommandType.Text, par);
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private Dictionary<String, Object> changemapping(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            try
            {
                ...
                //Exclui todos os pameamentos deste resource plugin
                database.ExecuteNonQuery("delete from resource_plugin_mapping where resource_plugin_id = @resource_plugin_id", CommandType.Text, par, trans);
                ...
                //foreach (Dictionary<String, Object> field in mapping)
                {
                    ...
                    Int64 fieldId = 0;
                    if ((field["field_id"] != null ) && (!String.IsNullOrWhiteSpace(field["field_id"].ToString())))
                    {
                        try
                        {
                    ...
                    DataTable dtField = database.ExecuteDataTable("select * from field where enterprise_id = @enterprise_id and id = " + fieldId, CommandType.Text, par, trans);
                    ...
                    database.ExecuteNonQuery( "insert into resource_plugin_mapping (resource_plugin_id, [field_id], [data_name], [is_id], [is_password], [is_property], [is_unique_property]) values (@resource_plugin_id, " + fieldId + ", '"  + dataName + "'," + (isId ? 1 : 0) + ", " + (isPassword ? 1 : 0) + ", " + (isProperty ? 1 : 0) + ", " + (isUnique ? 1 : 0) + ") ",  CommandType.Text,par, trans);

                }

                //Remove todos os campos que não foram listados
                //if (fieldList.Count > 0)
                //    database.ExecuteNonQuery( "delete from resource_plugin_mapping where resource_plugin_id = @resource_plugin_id and [field_id] not in (" + String.Join(",", fieldList) + ")",  CommandType.Text,par, trans);

                database.AddUserLog( LogKey.ResourcePluginMapping_Changed, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, 0, 0, "Resource x Plugin fields maping changed", String.Join("\r\n", log), trans);

                //Se o recurso x plugin estiver habilitado marca para que o proxy realize o download das configurações
                if ((Boolean)dr1["enabled"] && (Boolean)dr1["resource_enabled"])
                    database.ExecuteNonQuery( "update proxy set config = 1 where id = " + dr1["proxy_id"], CommandType.Text,par,  trans);
            ...
            {
                Error(ErrorType.InvalidRequest, "Error on update fields mapping", ex.Message, null);
                return null;
            }
            finally
            {
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private Boolean changelockrules(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            try
            {
                ...
                for (Int32 i = 0; i < lst.Count; i++)
                {
                    ...
                    DataTable dtRole = database.ExecuteDataTable( "select f.* from filters f with(nolock) where f.enterprise_id = @enterprise_id and f.id = " + lfId, CommandType.Text, par, trans);
                    ...
                    database.ExecuteNonQuery( "insert into resource_plugin_lock_filter (resource_plugin_id, filter_id) select @resource_plugin_id, @filter_id WHERE not exists (select 1 from resource_plugin_lock_filter with(nolock) where resource_plugin_id = @resource_plugin_id and filter_id = @filter_id)", CommandType.Text,parFilter,  trans);
                ...
                {
                    database.ExecuteNonQuery( "delete from resource_plugin_lock_filter where resource_plugin_id = @resource_plugin_id and filter_id not in (" + String.Join(",", filterList) + ")", CommandType.Text,parExp,  trans);
                }
                else
                {
                    database.ExecuteNonQuery( "delete from resource_plugin_lock_filter where resource_plugin_id = @resource_plugin_id", CommandType.Text,parExp,  trans);
                }

                database.AddUserLog( LogKey.ResourcePluginLockExpression_Changed, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, 0, 0, "Resource x Plugin lock rules changed", String.Join("\r\n", log), trans);
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private Boolean changeignore(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            try
            {
                ...
                for (Int32 i = 0; i < lst.Count; i++)
                {
                    ...
                    database.ExecuteNonQuery( "insert into resource_plugin_ignore_filter (resource_plugin_id, filter_id) select @resource_plugin_id, @filter_id WHERE not exists (select 1 from resource_plugin_ignore_filter with(nolock) where resource_plugin_id = @resource_plugin_id and filter_id = @filter_id)", CommandType.Text,parFilter,  trans);
                ...
                if (filterList.Count > 0)
                {
                    database.ExecuteNonQuery( "delete from resource_plugin_ignore_filter where resource_plugin_id = @resource_plugin_id and filter_id not in (" + String.Join(",", filterList) + ")", CommandType.Text,parExp,  trans);
                }
                else
                {
                    database.ExecuteNonQuery( "delete from resource_plugin_ignore_filter where resource_plugin_id = @resource_plugin_id", CommandType.Text,parExp,  trans);
                }
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private Boolean changeschedules(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            try
            {
                ...
                for (Int32 i = 0; i < lst.Count; i++)
                {
                    ...
                    database.ExecuteNonQuery( "insert into resource_plugin_schedule (resource_plugin_id, [schedule], next) select @resource_plugin_id, @schedule, getdate() WHERE not exists (select 1 from resource_plugin_schedule where resource_plugin_id = @resource_plugin_id and schedule = @schedule)", CommandType.Text,parSchedule,  trans);
                ...
                if (sheduleList.Count > 0)
                {
                    ...
                    database.ExecuteNonQuery( "delete from resource_plugin_schedule where resource_plugin_id = @resource_plugin_id and [schedule] not in (" + String.Join(",", k) + ")", CommandType.Text,parSchedule,  trans);
                }

                database.AddUserLog( LogKey.ResourcePluginLockSchedule_Changed, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, 0, 0, "Resource x Plugin schedule changed", String.Join("\r\n", log), trans);
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private Dictionary<String, Object> changerole(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            try
            {
                ...
                database.ExecuteNonQuery( "delete from resource_plugin_role where resource_plugin_id = @resource_plugin_id", CommandType.Text,par,  trans);
                ...
                //foreach (Dictionary<String, Object> field in mapping)
                {
                    ...
                    DataTable dtRole = database.ExecuteDataTable( "select r.* from role r with(nolock) inner join context c with(nolock) on c.id = r.context_id where c.enterprise_id = @enterprise_id and r.id = " + roleId, CommandType.Text, par, trans);
                    if ((dtRole == null) || (dtRole.Rows.Count == 0))
                    {
                    ...
                    /*
                    //Desabilitado a verificação para permitir roles sem nenhuma das 3 opções
                    ...
                    database.ExecuteNonQuery( "insert into resource_plugin_role (resource_plugin_id, role_id) select @resource_plugin_id, " + roleId + " WHERE not exists (select 1 from resource_plugin_role WHERE resource_plugin_id = @resource_plugin_id and [role_id] = " + roleId + ")", CommandType.Text,par,  trans);
                    ...
                    if (role.ContainsKey("actions"))
                    {
                        ...
                        database.ExecuteNonQuery( "delete from resource_plugin_role_action where resource_plugin_id = @resource_plugin_id and [role_id] = " + roleId, CommandType.Text,par,  trans);
                        ...
                        for (Int32 u = 0; u < actList.Count; u++)
                        {
                            ...
                            database.ExecuteNonQuery( "insert into resource_plugin_role_action (resource_plugin_id, [role_id], [action_key], [action_add_value], [action_del_value], [additional_data]) values (@resource_plugin_id, @role_id, @action_key, @action_add_value, @action_del_value, @additional_data) ", CommandType.Text,parAct,  trans);
                    ...
                    if (role.ContainsKey("filters"))
                    {
                        ...
                        database.ExecuteNonQuery( "delete from resource_plugin_role_filter where resource_plugin_id = @resource_plugin_id and [role_id] = " + roleId, CommandType.Text,par,  trans);
                        ...
                        for (Int32 u = 0; u < filterList.Count; u++)
                        {
                            ...
                            //Checa se o filtro existe
                            DataTable dtFilter = database.ExecuteDataTable( "select f.* from filters f with(nolock) where f.enterprise_id = @enterprise_id and f.id = " + lfId, CommandType.Text, par, trans);
                    ...
                    if (role.ContainsKey("time_acl"))
                    {
                        ...
                        database.ExecuteNonQuery( "delete from resource_plugin_role_time_acl where resource_plugin_id = @resource_plugin_id and [role_id] = " + roleId, CommandType.Text,par,  trans);
                        ...
                        for (Int32 u = 0; u < actList.Count; u++)
                        {
                            ...
                            database.ExecuteNonQuery( "insert into resource_plugin_role_time_acl (resource_plugin_id, [role_id], [time_acl]) values (@resource_plugin_id, @role_id, @time_acl) ", CommandType.Text,parTAcl,  trans);
                ...
                database.AddUserLog( LogKey.ResourcePluginRole_Changed, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, 0, 0, "Resource x Plugin roles changed", String.Join("\r\n", log), trans);
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private Dictionary<String, Object> changeparameters(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            try
            {
                ...
                foreach (String key in config.Keys)
                {
                    ...
                    if (val != null)
                    {
                        ...
                        database.ExecuteNonQuery( "delete from resource_plugin_par where resource_plugin_id = @resource_plugin_id and [key] = '" + key + "'", CommandType.Text,par,  trans);

                        using (DbParameterCollection par2 = new DbParameterCollection())
                        {
                            par2.Add("@resource_plugin_id", typeof(Int64)).Value = resourcepluginid;
                            par2.Add("@key", typeof(String)).Value = key;
                            par2.Add("@value", typeof(String)).Value = val;

                            database.ExecuteNonQuery("insert into resource_plugin_par (resource_plugin_id, [key], [value]) values (@resource_plugin_id, @key,@value) ", CommandType.Text, par2, trans);
                        }
                    }
                }


                database.AddUserLog( LogKey.ResourcePluginParameters_Changed, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, 0, 0, "Resource x Plugin plugin parameters changed", String.Join("\r\n", log), trans);
            ...
            {
                Error(ErrorType.InvalidRequest, "Error on update plugin parameters", ex.Message, null);
                return null;
            }
            finally
            {
        ...
        private Dictionary<String, Object> change(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            foreach (String key in parameters.Keys)
                switch (key)
                {
                    ...
                    case "resourceid":
                        if (!String.IsNullOrWhiteSpace((String)parameters["resourceid"]))
                        {
                            ...
                            if (resourceid.ToString() != dtResourcePlugin.Rows[0]["resource_id"].ToString())
                            {
                                ...
                                DataTable dtRes = database.ExecuteDataTable( "select r.* from resource r with(nolock) inner join context c with(nolock) on c.id = r.context_id where c.enterprise_id = @enterprise_id and r.id = " + resourceid, CommandType.Text, par, null);
                    ...
                    case "pluginid":
                        if (!String.IsNullOrWhiteSpace((String)parameters["pluginid"]))
                        {
                            ...
                            if (pluginid.ToString() != dtResourcePlugin.Rows[0]["plugin_id"].ToString())
                            {
                                ...
                                //Verifica se o recurso existe e pertence a mesma empresa
                                DataTable dtPlug = database.ExecuteDataTable( "select * from plugin p with(nolock) where p.enterprise_id in (0,@enterprise_id) and p.id = " + pluginid, CommandType.Text, par, null);
                    ...
                    case "name_field_id":
                        if (!String.IsNullOrWhiteSpace((String)parameters["name_field_id"]))
                        {
                            ...
                            if (name_field_id.ToString() != dtResourcePlugin.Rows[0]["name_field_id"].ToString())
                            {
                                ...
                                //Verifica se o recurso existe e pertence a mesma empresa
                                DataTable dtField = database.ExecuteDataTable( "select * from field f where f.enterprise_id = @enterprise_id and f.id = " + name_field_id, CommandType.Text, par, null);
                                if ((dtField == null) || (dtField.Rows.Count == 0))
                                {
                    ...
                    case "mail_field_id":
                        if (!String.IsNullOrWhiteSpace((String)parameters["mail_field_id"]))
                        {
                            ...
                            if (mail_field_id.ToString() != dtResourcePlugin.Rows[0]["mail_field_id"].ToString())
                            {
                                ...
                                //Verifica se o recurso existe e pertence a mesma empresa
                                DataTable dtField = database.ExecuteDataTable( "select * from field f where f.enterprise_id = @enterprise_id and f.id = " + mail_field_id, CommandType.Text, par, null);
                                if ((dtField == null) || (dtField.Rows.Count == 0))
                                {
                    ...
                    case "login_field_id":
                        if (!String.IsNullOrWhiteSpace((String)parameters["login_field_id"]))
                        {
                            ...
                            if (login_field_id.ToString() != dtResourcePlugin.Rows[0]["login_field_id"].ToString())
                            {
                                ...
                                //Verifica se o recurso existe e pertence a mesma empresa
                                DataTable dtField = database.ExecuteDataTable( "select * from field f where f.enterprise_id = @enterprise_id and f.id = " + login_field_id, CommandType.Text, par, null);
                                if ((dtField == null) || (dtField.Rows.Count == 0))
                                {
                                    Error(ErrorType.InvalidRequest, "New mail field not exists or is not a chield of this enterprise.", "", null);
                                    return null;
                    ...
                    case "import_groups":
                        if (!String.IsNullOrWhiteSpace((String)parameters["import_groups"].ToString()))
                        {
                            Boolean import_groups = false;
                            try
                            {
                                import_groups = Boolean.Parse(parameters["import_groups"].ToString());
                            }
                            catch
                            {
                                Error(ErrorType.InvalidRequest, "Parameter import_groups is not a boolean.", "", null);
                                return null;
                            }

                            if (import_groups != (Boolean)dtResourcePlugin.Rows[0]["import_groups"])
                            {
                                par.Add("@import_groups", typeof(Boolean)).Value = import_groups;
                                if (updateSQL != "") updateSQL += ", ";
                                updateSQL += " import_groups = @import_groups";
                                update = true;

                                log.Add("Import groups changed from '" + dtResourcePlugin.Rows[0]["import_groups"] + "' to '" + import_groups + "'");
                            }
                    ...
                    case "import_containers":
                        if (!String.IsNullOrWhiteSpace((String)parameters["import_containers"].ToString()))
                        {
                            Boolean import_containers = false;
                            try
                            {
                                import_containers = Boolean.Parse(parameters["import_containers"].ToString());
                            }
                            catch
                            {
                                Error(ErrorType.InvalidRequest, "Parameter import_containers is not a boolean.", "", null);
                                return null;
                            }

                            if (import_containers != (Boolean)dtResourcePlugin.Rows[0]["import_containers"])
                            {
                                par.Add("@import_containers", typeof(Boolean)).Value = import_containers;
                                if (updateSQL != "") updateSQL += ", ";
                                updateSQL += " import_containers = @import_containers";
                                update = true;

                                log.Add("Import containers changed from '" + dtResourcePlugin.Rows[0]["import_containers"] + "' to '" + import_containers + "'");
                            }

                        }
                        break;

                    case "use_password_salt":
                        if (!String.IsNullOrWhiteSpace((String)parameters["use_password_salt"].ToString()))
                        {
                            Boolean use_password_salt = false;
                            try
                            {
                                use_password_salt = Boolean.Parse(parameters["use_password_salt"].ToString());
                            }
                            catch
                            {
                                Error(ErrorType.InvalidRequest, "Parameter use_password_salt is not a boolean.", "", null);
                                return null;
                            }

                            if (use_password_salt != (Boolean)dtResourcePlugin.Rows[0]["use_password_salt"])
                            {
                                par.Add("@use_password_salt", typeof(Boolean)).Value = use_password_salt;
                                if (updateSQL != "") updateSQL += ", ";
                                updateSQL += " use_password_salt = @use_password_salt";
                                update = true;

                                log.Add("Use password salt changed from '" + dtResourcePlugin.Rows[0]["use_password_salt"] + "' to '" + use_password_salt + "'");
                            }

                        }
                        break;

                    case "password_salt_end":
                        if (!String.IsNullOrWhiteSpace((String)parameters["password_salt_end"].ToString()))
                        {
                            Boolean password_salt_end = false;
                            try
                            {
                                password_salt_end = Boolean.Parse(parameters["password_salt_end"].ToString());
                            }
                            catch
                            {
                                Error(ErrorType.InvalidRequest, "Parameter password_salt_end is not a boolean.", "", null);
                                return null;
                            }

                            if (password_salt_end != (Boolean)dtResourcePlugin.Rows[0]["password_salt_end"])
                            {
                                par.Add("@password_salt_end", typeof(Boolean)).Value = password_salt_end;
                                if (updateSQL != "") updateSQL += ", ";
                                updateSQL += " password_salt_end = @password_salt_end";
                                update = true;

                                log.Add("Password salt on end changed from '" + dtResourcePlugin.Rows[0]["password_salt_end"] + "' to '" + password_salt_end + "'");
                            }
                    ...
                    case "password_salt":
                        if (!String.IsNullOrWhiteSpace((String)parameters["password_salt"].ToString()))
                        {
                            String password_salt = "";
                            try
                            {
                                password_salt = parameters["password_salt"].ToString();
                            }
                            catch
                            {
                                Error(ErrorType.InvalidRequest, "Parameter password_salt is null or empty.", "", null);
                                return null;
                            }

                            if (password_salt != (dtResourcePlugin.Rows[0]["password_salt"] == System.DBNull.Value ? "" : (String)dtResourcePlugin.Rows[0]["password_salt"]))
                            {
                                par.Add("@password_salt", typeof(String)).Value = password_salt;
                                if (updateSQL != "") updateSQL += ", ";
                                updateSQL += " password_salt = @password_salt";
                                update = true;

                                log.Add("Password salt changed from '" + dtResourcePlugin.Rows[0]["password_salt"] + "' to '" + password_salt + "'");
                            }
            ...
            if (update)
            {
                ...
                database.AddUserLog( LogKey.ResourcePlugin_Changed, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, 0, 0, "Resource x Plugin changed", String.Join("\r\n", log));


                //Se o recurso x plugin estiver habilitado marca para que o proxy realize o download das configurações
                if ((Boolean)dtResourcePlugin.Rows[0]["enabled"] && (Boolean)dtResourcePlugin.Rows[0]["resource_enabled"])
                    database.ExecuteNonQuery( "update proxy set config = 1 where id = " + dtResourcePlugin.Rows[0]["proxy_id"], CommandType.Text, par);

            }

            //Atualiza a busca com os dados atualizados
            dtResourcePlugin = database.ExecuteDataTable( "SELECT rp.*, (r.name + ' x ' + p.name) as name, r.proxy_id, r.enabled resource_enabled, r.name resource_name, p.name plugin_name, f1.name name_field_name, f2.name mail_field_name, f3.name login_field_name FROM resource_plugin rp with(nolock) INNER JOIN resource r with(nolock) ON r.id = rp.resource_id INNER JOIN plugin p with(nolock) ON p.id = rp.plugin_id INNER JOIN context c with(nolock) ON r.context_id = c.id left join field f1 on f1.id = rp.name_field_id left join field f2 on f2.id = rp.mail_field_id left join field f3 on f3.id = rp.login_field_id WHERE rp.id = @resource_plugin_id AND c.enterprise_id = @enterprise_id", CommandType.Text, par, null);
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private List<Dictionary<String, Object>> list(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            List<Dictionary<String, Object>> result = new List<Dictionary<String, Object>>();
            ...
            if ((dtRoles != null) && (dtRoles.Rows.Count > 0))
            {
                ...
                foreach (DataRow dr1 in dtRoles.Rows)
                {
                    ...
                    Dictionary<string, object> newItem = get(database, param, true);
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private List<Dictionary<String, Object>> identity(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            List<Dictionary<String, Object>> result = new List<Dictionary<String, Object>>();
        ...
        /// <param name="sqlConnection">Conexão com o banco de dados MS-SQL</param>
        /// <param name="parameters">Dicionário (String, Object) contendo todos os parâmetros necessários</param>
        private Boolean addidentity(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            DataTable dtResourcePluginIds = database.ExecuteDataTable( "select * from resource_plugin_mapping rpm with(nolock) WHERE (is_id = 1 or is_unique_property = 1) AND rpm.resource_plugin_id = @resource_plugin_id", CommandType.Text, par, null);
            ...
            try
            {
                ...
                foreach (Int64 u in users)
                {
                    ...
                    DataTable dtEntity = database.ExecuteDataTable( "select distinct e.* from entity e with(nolock) inner join resource r with(nolock) on e.context_id = r.context_id inner join resource_plugin rp with(nolock) on r.id = rp.resource_id WHERE e.id = " + u, CommandType.Text, par, trans);
                    ...
                    DataTable dtNewIdentity = database.ExecuteDataTable( "sp_new_identity", CommandType.StoredProcedure, parId, trans);
                    ...
                    if ((Boolean)dtNewIdentity.Rows[0]["new_identity"])
                    {
                        ...
                        if (idFields.Count > 0)
                        {
                            ...
                            if ((dtIfe != null) && (dtIfe.Rows.Count > 0))
                            {
                                ...
                                database.ExecuteNonQuery( "insert into identity_field (identity_id, field_id, value) SELECT @identity_id, @field_id, @value WHERE not exists (select 1 from identity_field where identity_id = @identity_id and field_id = @field_id)", CommandType.Text,par2,  trans);
                        ...
                        database.AddUserLog( LogKey.User_IdentityNew, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, u, identityId, "Identity added on resource x plugin " + dtResourcePlugin.Rows[0]["name"], String.Join("\r\n", log), trans);
                    }
                    else
                    {
                ...
                database.AddUserLog( LogKey.ResourcePluginIdentity_Changed, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, 0, 0, "Identity added", String.Join("\r\n", logAll), trans);
        ...
        private Boolean deleteidentity(IAMDatabase database, Dictionary<String, Object> parameters)
        {
            ...
            DataTable dtResource = database.ExecuteDataTable( "select rp.*, (r.name + ' x ' + p.name) as name from resource_plugin rp with(nolock) inner join resource r with(nolock) on r.id = rp.resource_id inner join plugin p with(nolock) on p.id = rp.plugin_id inner join context c with(nolock) on c.id = r.context_id WHERE rp.id = @resource_plugin_id AND c.enterprise_id = @enterprise_id", CommandType.Text, par, null);
            if (dtResource == null)
            {
                Error(ErrorType.InternalError, "", "", null);
                return false;
            ...
            DataTable dtIdentity = database.ExecuteDataTable( "select i.* from [identity] i with(nolock) inner join entity e with(nolock) on e.id = i.entity_id inner join resource_plugin rp with(nolock) on rp.id = i.resource_plugin_id WHERE rp.id = @resource_plugin_id and e.id = @entity_id", CommandType.Text, par, null);
            ...
            try
            {
                ...
                foreach (DataRow dr in dtIdentity.Rows)
                    ...
                    {
                        database.AddUserLog( LogKey.User_IdentityDeleted, null, "API", UserLogLevel.Info, 0, this._enterpriseId, 0, 0, 0, (Int64)dr["entity_id"], (Int64)dr["id"], "Identity deleted of resource x plugin " + dtResource.Rows[0]["name"], "");
                        database.ExecuteNonQuery( "delete from [identity] where id = " + dr["id"], CommandType.Text, par);
